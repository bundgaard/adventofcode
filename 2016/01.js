function run(input) {
	let mapDirection = input.replace(/\s/gi, "").split(",")
	console.log(mapDirection);
	let direction = 0;
	mapDirection.forEach(element => {
		let [orientation, steps] = element.split("");
		console.log(`${orientation}, ${steps}`);
		if (orientation === "R") {
			direction += parseInt(steps);
